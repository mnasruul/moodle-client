...
func Test_courseAPI_GetCoursesByField(t *testing.T) {
	t.Parallel()

	type args struct {
		ctx   context.Context
		param CoursesByFieldOptions
	}
	tests := []struct {
		name     string
		args     args
		...
		{
			name: "Successful response",
			args: args{ctx: context.Background(), param: CoursesByFieldOptions{CourseFieldValues: &CourseFieldValues{{Field: CourseFieldCategory, Value: "2"}}}},
			response: `{
  "courses": [
    {
      "id": 1111,
  ...
  "nextoffset": 2
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := c.GetCoursesByField(tt.args.ctx, tt.args.param)
			if (err != nil) != tt.wantErr {
				t.Errorf("GetCoursesByField() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if diff := cmp.Diff(got, tt.want); diff != "" {
				t.Errorf("GetEnrolledCoursesByTimelineClassification() (-got, +want)\n%s", diff)
